...
public class IceCreamParlorService {
    ...
    public Sundae getSundae(List<String> flavorNames) {
        ...
        // cartons.removeIf(carton   -> carton.isEmpty());  // () not needed around param if only one
           cartons.removeIf(Carton::isEmpty);     // Use ClassName:methodName method reference
    ...
    Sundae buildSundae(List<Carton> cartons) {
        ...
        // Go through the liost of cartons calling each ericsCarton
        // Use ericCarton to get add a scoop of whatver flavor it is
        cartons.forEach(ericsCarton -> sundae.addScoop(ericsCarton.getFlavor()));
       // cartons.forEach(Sundae::addscoop(getFlavor()));
    ...
    public int prepareFlavors(List<String> flavorNames) {
        ...
        //          Use a helper (see below) called map() to make this work
        //
        // replaced new ArrayList with the resullty of calling the map() method
        // List<Queue<Ingredient>> ingredientQueues = new ArrayList<>();

        // Use map() helper method with Lambda expression to get List<Queue<Ingredients>
        //List<Queue<Ingredient>> ingredientQueues = map(recipes, (aRecipe) -> RecipeConverter.fromRecipeToIngredientQueue(aRecipe));

        // Use stream.map() method with Lambda expression to get List<Queue<Ingredients>
     ...
     * like this.)
     *
     * Provides an generic wrapper into the stream.map() methods
     *
     */
    //               receive a List         , a function (or lambda expression which is a function)
    private <T, R> List<R> map(List<T> input, Function<T, R> converter) {
        return input.stream()
                    .map(converter) // run the stream() .map method with the lambda expression passed to it
            .collect(Collectors.toList());
    }
}
